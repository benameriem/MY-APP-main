...
export class Dashboard implements OnInit {
  ...
  // Section active
  activeSection: 'dashboard' | 'liste' | 'transport' | 'logistique' | 'global' = 'dashboard';
  // Thème (minuit par défaut)
  theme: 'midnight' | 'light' = 'midnight';
  @HostBinding('class.theme-midnight') get isMidnight() { return this.theme === 'midnight'; }
  ...
  ngOnInit() {
    if (typeof localStorage !== 'undefined') {
      const savedTheme = (localStorage.getItem('theme') as 'midnight' | 'light' | null);
  ...
  loadApplicationsDirectly() {
    ...
    console.log('✅ Applications chargées en mode fallback:', this.applications);
    this.cdr.detectChanges();
  }

  createApplication() {
    if (!this.isAdmin) { alert('Action réservée aux administrateurs'); return; }
  ...
  updateApplication() {
    ...
    if (this.editingApp && this.newApp.name && this.newApp.department) {
      this.applicationService.update(this.editingApp.id!, this.newApp).subscribe({
        next: () => {
          this.loadApplications();
          this.resetForm();
          this.pendingChanges = true;
  ...
  getFilteredApplications(): Application[] {
    ...
    return this.applications.filter(app => {
      ...
      const matchesDept = this.departmentFilter === 'all' || app.department === this.departmentFilter;
  ...
  toggleStatus(app: Application) {
    if (!this.isAdmin) { alert('Action réservée aux administrateurs'); return; }
    const next: 'active' | 'inactive' = app.status === 'active' ? 'inactive' : 'active';
    const payload = {
      name: app.name,
      theme: app.theme,
      performance: app.performance,
  ...
  refreshData(triggerExport = true) {
    ...
    this.applicationService.relance({ reason: this.pendingChanges ? 'apply-changes' : 'manual', pending: this.pendingChanges })
      .subscribe({
        next: () => {
  ...
  getLastSyncTime(): string {
    const now = new Date();
    const diff = Math.floor((now.getTime() - this.lastSyncTime.getTime()) / 1000);
  ...
  exportDoc() {
    const rows = this.getSectionApps().map(a => `
      <tr>
        <td>${a.name || ''}</td>
        <td>${a.theme || ''}</td>
        <td>${a.department || ''}</td>
  ...
  setActiveSection(section: 'dashboard' | 'liste' | 'transport' | 'logistique' | 'global') {
  ...
  getAveragePerformance(): string {
    ...
    const sum = apps.reduce((acc, app) => acc + (perfMap[app.performance || 'Moyen'] || 2), 0);
